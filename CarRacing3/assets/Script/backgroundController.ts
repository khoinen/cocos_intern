...
export default class NewClass extends cc.Component {
    ...
    @property (cc.Prefab)
    otherCar1 : cc.Prefab = null;

    @property (cc.Prefab)
    otherCar2: cc.Prefab = null;

    @property (cc.Node)
    carsNode: cc.Node = null;
    ...
    onLoad () {
        this.spawnRoads();
        this.spawnUsingPrefabs(this.otherCar2, this.carsNode, -350, -140);
        this.spawnUsingPrefabs(this.otherCar1, this.carsNode, 350, 140);
        this.carsNode.active = false;
    }
    ...
    update (dt) {
        if (this.game.isStarted && !this.hasCrossRoad && this.findTopIndex(this.node.children) === this.numberRoad - 1) {
            ...
            let crossroadNode = this.spawnUsingPrefabs(this.crossroad, this.node, 0, nPosY);
            this.carsNode.setPosition(crossroadNode.getPosition());

            this.carsNode.active = true;
            this.numberRoad += 1;
        ...
        if (!this.endAtCrossRoad){
            this.node.children.forEach((road, index) => {
                ...
                if (this.hasCrossRoad && index === this.numberRoad - 1 && road.getBoundingBoxToWorld().yMax <=  this.game.node.height){
